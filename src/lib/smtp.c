...
static char* state_names[] = {"SERVER_GREETING","SERVER_UNRECOGNIZED_CMD", "SERVER_INVALID_MAIL_CMD", "SERVER_INVALID_HELO_CMD", "SERVER_INVALID_EHLO_CMD", "SERVER_INVALID_RCPT_CMD", "SERVER_INVALID_STAT_CMD",
                             ...
                             "CLIENT_STAT_BYTES_TRANSFERED","SERVER_STAT_BYTES_TRANSFERED", "CLIENT_STAT_MAILS_SENT","SERVER_STAT_MAILS_SENT", "SERVER_ALREADY_GREETED", "SERVER_NO_MAIL", "CLIENT_MAIL_FROM", "SERVER_ALREADY_MAIL",
...
static unsigned server_stat_mails_sent(struct selector_key *key);
...
static const struct state_definition client_statbl[] = {
    ...
    {
        .state            = CLIENT_STAT_MAILS_SENT,
        .on_arrival       = NULL,
        .on_departure     = NULL,
        .on_read_ready    = client_read,
    },
    {
        .state            = SERVER_STAT_MAILS_SENT,
        .on_arrival       = NULL,
        .on_departure     = NULL,
        .on_write_ready   = server_stat_mails_sent
    },
    {
...
static unsigned server_help(struct selector_key * key) {
    return server_template(key,
        ...
        "QUIT\n"
        "VRFY\n"
        "XSTAT <TOTAL|CURRENT|BYTES|MAILS>\n",
        status_action_okay,
        GOTO_PREVIOUS
    );
}
...
static unsigned server_stat_active_connections(struct selector_key *key) {
    return server_template(key, SERVER_STAT_CURRENT_CONNECTIONS, "current connections: %d\n", current_connections, GOTO_PREVIOUS);
}

static unsigned server_stat_historic_connections(struct selector_key *key) {
    return server_template(key, SERVER_STAT_TOTAL_CONNECTIONS, "total connections: %d\n", total_connections, GOTO_PREVIOUS);
}

static unsigned server_stat_bytes_transfered(struct selector_key *key) {
    return server_template(key, SERVER_STAT_BYTES_TRANSFERED, "total bytes of data transferred: %d\n", transferred_bytes, GOTO_PREVIOUS);
}

static unsigned server_stat_mails_sent(struct selector_key *key) {
    return server_template(key, SERVER_STAT_MAILS_SENT, "total mails sent: %d\n", total_mails_sent, GOTO_PREVIOUS);
}
